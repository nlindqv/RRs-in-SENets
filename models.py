...
class ResNet(tf.keras.Model):
    ...
    '''
    def __init__(self, filters=(16, 32, 64), kernel_sizes=(3, 3, 3), ratios=(16, 16, 16), se_block=False):
        ...
        self.se_block1 = SE_block(ratio=16, channels=16)

        self.stage_lengths = [3, 3, 3]
        self.main_layers = []

        # stage 1
        for layer in range(self.stage_lengths[0]):
            self.main_layers.append(RES_block(n_filter=filters[0], kernel_size=kernel_sizes[0], se_block=se_block))

        # stage 2
        self.main_layers.append(RES_block(n_filter=filters[1], kernel_size=kernel_sizes[1], first_block=True, se_block=se_block))
        for layer in range(self.stage_lengths[1]-1):
            self.main_layers.append(RES_block(n_filter=filters[1], kernel_size=kernel_sizes[1], se_block=se_block))

        # stage 3
        self.main_layers.append(RES_block(n_filter=filters[2], kernel_size=kernel_sizes[2], first_block=True, se_block=se_block))
        for layer in range(self.stage_lengths[2]-1):
            self.main_layers.append(RES_block(n_filter=filters[2], kernel_size=kernel_sizes[2], se_block=se_block))
    ...
    def call(self, inputs, training=False, mask=None):
        ...
        current = self.batch_norm1(current)
...
class RES_block(tf.keras.layers.Layer):
    def __init__(self, n_filter, kernel_size, first_block=False, se_block=False):
        super(RES_block, self).__init__()
        self.first_block = first_block

        self.cnn_block = CNN_block(filters=n_filter, kernal_size=kernel_size, se_block=se_block, first_block=first_block)
        if self.first_block: # if it is the first layer of a stage, then the residual needs to change dims, which is done here.
            self.residual_layer = tf.keras.layers.Conv2D(filters=n_filter, kernel_size=1, strides=2, activation=None)

    @tf.function
    def call(self, inputs, training=False, mask=None):
        current = self.cnn_block(inputs)
        if self.first_block:
...
class CNN_block(tf.keras.layers.Layer):
    def __init__(self, filters=16, kernal_size=3, se_block=False, se_ratio=16, first_block=False):
        ...
        if self.se_block:
            ...
            self.se_block2 = SE_block(ratio=se_ratio, channels=filters)
    ...
    def call(self, inputs, training=False, mask=None):
        ...
        current = self.batch_norm2(current)
        if self.se_block:
            current =self.se_block1(current)
